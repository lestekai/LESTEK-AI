const fs = require('fs');
let code = fs.readFileSync('coreExercise.tsx', 'utf8');

// Strip the beginning up to the OBJETIVO DA SÉRIE div
const objIdx = code.indexOf('🎯 OBJETIVO DA SÉRIE');
let startContent = code.lastIndexOf('<div', objIdx);

// Strip the end (</motion.div>, )}, </AnimatePresence>)
let lines = code.substring(startContent).split('\n');
// We want to remove the last 3 lines if they match our unwanted tags
while (lines.length > 0 && (lines[lines.length - 1].trim() === '</AnimatePresence>' || lines[lines.length - 1].trim() === ')}' || lines[lines.length - 1].trim() === '</motion.div>')) {
  lines.pop();
}
...
fs.writeFileSync('newExerciseRender2.tsx', finalCode);
