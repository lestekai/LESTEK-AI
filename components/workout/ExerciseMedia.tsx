...
export const ExerciseMedia = ({
  ...
  const handleMediaError = (failedUrl?: string) => {
    const targetUrl = failedUrl || resolvedMedia?.url;
    if (targetUrl) {
       console.error('Falhou ao carregar mídia, buscando fallback:', targetUrl, exerciseNameOrId);
       
       // Track this failure so we don't try it again
       markMediaAsFailed(targetUrl);
       ...
       if (nextMedia.url && nextMedia.url !== targetUrl) {
          ...
          if (nextMedia.type === 'video') {
            fetch(nextMedia.url)
              ...
              .catch(() => {
                 markMediaAsFailed(nextMedia.url);
                 setHasError(true);
                 setIsLoading(false);
              });
          }
    ...
    <div className={`relative flex items-center justify-center overflow-hidden bg-surface ${className}`}>
      ...
      {hasError ? (
        ...
        blobUrl ? (
          ...
          >
            <source src={blobUrl} type={resolvedMedia.url.endsWith(".webm") ? "video/webm" : "video/mp4"} />
          </video>
        ) : null
      ) : (
        <img
