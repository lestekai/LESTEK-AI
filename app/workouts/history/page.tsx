...
export default function HistoryPage() {
  ...
  const { workoutHistory, currentPlan } = useWorkoutStore();
  ...
  return (
    <div className="min-h-screen bg-background pb-32 relative overflow-x-hidden text-white font-sans">
      <header className="p-5 sticky top-0 bg-background/95 backdrop-blur-xl z-30 flex items-center justify-between border-b border-white/5">
      ...
      <main className="p-5 max-w-2xl mx-auto relative z-10 space-y-6">
        {/* Sleek Tabs Navigation */}
        <div className="flex bg-surface p-1.5 rounded-[20px] border border-white/5 overflow-x-auto scrollbar-hide gap-1 sticky top-[80px] z-20">
          {navItems.map((item) => (
            <button 
              key={item.id}
        ...
        {workoutHistory.length === 0 ? (
          <div className="bg-surface/30 backdrop-blur-md border border-white/5 border-dashed p-10 rounded-[32px] text-center flex flex-col items-center justify-center mt-8">
            <div className="w-24 h-24 bg-surface border border-white/10 rounded-full flex items-center justify-center mb-6 shadow-xl">
              <Dumbbell size={40} className="text-slate-500/50" />
            </div>
            <h2 className="text-2xl font-black text-white mb-2">Sem histórico</h2>
            <p className="text-sm text-slate-400 max-w-[280px] leading-relaxed mx-auto font-medium">
              Sua evolução começará a ser exibida assim que concluir seu primeiro treino.
            </p>
            <button 
              onClick={() => navigate('/workouts')}
              className="mt-8 px-8 py-4 bg-gradient-to-r from-[#e57d3b] to-amber-500 text-black text-sm font-black uppercase tracking-widest rounded-full hover:scale-105 active:scale-95 transition-all shadow-lg shadow-amber-500/20"
            >
              Começar Treino
            </button>
          </div>
          ...
          <AnimatePresence mode="wait">
                ...
                {[...workoutHistory].reverse().map((log, i) => {
                  ...
                  return (
                    <motion.div 
                      ...
                      className="bg-surface border border-white/5 rounded-[24px] overflow-hidden transition-all hover:border-white/10 cursor-pointer shadow-lg"
                      onClick={() => setExpandedIndex(isExpanded ? null : i)}
                    >
                      <div className="p-5 flex items-center justify-between">
                        <div className="flex items-center gap-4">
                          <div className="w-14 h-14 bg-background border border-white/5 rounded-[20px] flex flex-col items-center justify-center shrink-0 shadow-inner">
                            <span className="text-xl font-black text-white leading-none">{d.getDate()}</span>
                          ...
                          <div>
                            <h3 className="font-bold text-white text-base tracking-tight mb-1">{log.dayFocus}</h3>
                            <div className="flex items-center gap-3 text-xs font-bold text-slate-400">
                              <span className="flex items-center gap-1.5"><Clock size={12} className="text-slate-500" /> {log.durationMinutes}m</span>
                              <span className="flex items-center gap-1.5"><Activity size={12} className="text-slate-500" /> {log.exercisesCompleted} exs</span>
                            </div>
                          </div>
                        </div>
                            ...
                            <div className="p-5 space-y-4">
                              {log.exerciseLogs && log.exerciseLogs.length > 0 ? (
                                <div className="space-y-3">
                                  <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest block mb-2">Exercícios Executados</span>
                                  {log.exerciseLogs.map((el, elIdx) => (
                                    <div key={elIdx} className="flex justify-between items-center gap-2 py-2 border-b border-white/5 last:border-0 last:pb-0">
                                      <span className="text-sm text-white font-bold max-w-[65%] truncate">
                                        {el.exerciseName}
                                      </span>
                                ...
                                <p className="text-xs text-slate-500 font-medium italic">Detalhes não encontrados.</p>
                              )}
                            </div>
                          </motion.div>
                        )}
            ...
            {activeTab === 'calendar' && (
              ...
              >
                <div className="grid grid-cols-3 gap-3">
                  <div className="bg-surface border border-white/5 p-4 rounded-[24px] text-center shadow-lg">
                    <span className="block text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1 mt-1">Treinos</span>
                    <span className="text-3xl font-black text-white">{calendarMetrics.completedThisMonth}</span>
                  </div>
                  <div className="bg-surface border border-white/5 p-4 rounded-[24px] text-center shadow-lg">
                    <span className="block text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1 mt-1">Frequência</span>
                    <span className="text-3xl font-black text-emerald-400">{calendarMetrics.frequencyRate}%</span>
                  </div>
                  <div className="bg-surface border border-white/5 p-4 rounded-[24px] text-center shadow-lg">
                    <span className="block text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1 mt-1">Faltas</span>
                    <span className="text-3xl font-black text-red-400">{calendarMetrics.absencesCount}</span>
                  </div>
                </div>

                <div className="bg-surface border border-white/5 p-5 sm:p-6 rounded-[32px] space-y-6 shadow-xl">
                  <div className="flex justify-between items-center bg-background/50 p-2 rounded-2xl border border-white/5">
                    <button onClick={prevMonth} className="w-10 h-10 flex items-center justify-center bg-surface hover:bg-white/10 text-white rounded-xl transition-colors">
                      <ChevronLeft size={18} />
                    </button>
                    <h3 className="font-black text-sm uppercase tracking-wider text-white">
                      {monthNames[calendarMonth]} <span className="text-[#e57d3b]">{calendarYear}</span>
                    </h3>
                    <button onClick={nextMonth} className="w-10 h-10 flex items-center justify-center bg-surface hover:bg-white/10 text-white rounded-xl transition-colors">
                      <ChevronRight size={18} />
                    </button>
                  </div>

                  <div className="grid grid-cols-7 gap-2 text-center font-black text-[10px] text-slate-500 uppercase tracking-widest mb-2">
                      ...
                      return (
                        <button
                          ...
                          className={`aspect-square rounded-2xl text-[11px] font-black flex flex-col items-center justify-center relative transition-all ${
                            ...
                            'bg-background border border-white/5 text-slate-500 hover:border-white/20'
                          }`}
                        >
                          {dayItem.day}
                        </button>
                ...
                <AnimatePresence mode="wait">
                  {selectedCalendarDate && clickedDayWorkout && (
                    <motion.div
                      ...
                      initial={{ opacity: 0, y: 15 }}
                      animate={{ opacity: 1, y: 0 }}
                      className="bg-[#e57d3b] text-black p-6 rounded-[32px] shadow-lg"
                    >
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="grid grid-cols-2 lg:grid-cols-3 gap-2">
                          {clickedDayWorkout.exerciseLogs?.map((ex, i) => (
                            <div key={i} className="bg-white/20 px-3 py-2 rounded-xl text-xs font-black truncate border border-white/10 shadow-sm text-black">
                              {ex.exerciseName}
                            </div>
                          ))}
                        </div>
            ...
            {activeTab === 'metrics' && (
              ...
              >
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  <div className="bg-surface border border-white/5 p-6 rounded-[24px] flex items-center justify-between shadow-lg">
                    <div>
                      <span className="block text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Volume Hoje</span>
                      <span className="text-2xl font-black text-white">{statsVolume.dailyVolume.toLocaleString('pt-BR')} <span className="text-xs text-slate-500 uppercase tracking-widest">kg</span></span>
                    </div>
                    <Flame size={28} className="text-[#e57d3b]" />
                  </div>
                  <div className="bg-surface border border-white/5 p-6 rounded-[24px] flex items-center justify-between shadow-lg">
                    <div>
                      <span className="block text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Vol. Semanal</span>
                      <span className="text-2xl font-black text-white">{statsVolume.weeklyVolume.toLocaleString('pt-BR')} <span className="text-xs text-slate-500 uppercase tracking-widest">kg</span></span>
                    </div>
                    <Zap size={28} className="text-amber-500" />
                  </div>
                  <div className="bg-surface border border-white/5 p-6 rounded-[24px] flex items-center justify-between shadow-lg">
                    <div>
                      <span className="block text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Vol. Mensal</span>
                ...
                <div className="bg-surface border border-white/5 p-6 rounded-[32px] space-y-6 shadow-xl">
                  <div>
                  ...
                  <div className="space-y-5">
                    {muscleGroupsStats.length === 0 ? (
                      <p className="text-xs text-slate-500 font-bold mb-4">Sem dados musculares.</p>
                    ) : (
                        ...
                        return (
                          <div key={idx} className="space-y-2">
                            <div className="flex justify-between items-end text-xs">
                              <span className="font-black text-white tracking-wide">{item.muscle}</span>
                              <div className="flex gap-3 text-[10px] font-black uppercase tracking-widest text-slate-500">
                                <span><span className="text-white">{item.sets}</span> s</span>
            ...
            {activeTab === 'history_chart' && (
              <motion.div
                ...
                className="space-y-6"
              >
                <div className="bg-surface border border-white/5 p-6 rounded-[32px] space-y-8 shadow-xl">
                  <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
                    <div className="w-full">
                      <h3 className="text-xs font-black uppercase tracking-widest text-white mb-3">Evolução de Cargas</h3>
                      {uniqueExercisesInHistory.length > 0 && (
                        <select
                          value={activeExToUse}
                          onChange={(e) => setSelectedEvolutionEx(e.target.value)}
                          className="bg-background border border-white/10 px-4 py-3.5 rounded-[20px] text-xs font-black text-[#e57d3b] uppercase tracking-wider outline-none w-full cursor-pointer hover:border-white/20 transition-all appearance-none"
                        >
                          {uniqueExercisesInHistory.map((ex, idx) => (
                            <option key={idx} value={ex} className="font-bold bg-surface text-white">{ex}</option>
                          ))}
                        </select>
                      )}
                    </div>
                  ...
                  {activeExToUse ? (
                    <div className="space-y-6">
                       <div className="grid grid-cols-2 gap-3">
                        <div className="bg-background/80 p-4 rounded-2xl border border-white/5">
                          <span className="block text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">Max Carga</span>
                          <span className="text-2xl font-black text-white">{exerciseEvolutionData.length > 0 ? Math.max(...exerciseEvolutionData.map(d => d.maxWeight)) : 0} <span className="text-[10px] text-slate-500 uppercase tracking-widest">kg</span></span>
                        </div>
                         <div className="bg-background/80 p-4 rounded-2xl border border-white/5">
                          <span className="block text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">Max Volume</span>
                      ...
                      {exerciseEvolutionData.length > 0 ? (
                        ...
                        <div className="p-8 border border-white/10 border-dashed rounded-[24px] text-center text-[10px] font-black text-slate-500 uppercase tracking-widest">
                          Dados insuficientes.
                        </div>
                      )}
                    </div>
                  ) : (
                    <div className="p-8 text-center text-[10px] font-black text-slate-500 uppercase tracking-widest border border-white/10 border-dashed rounded-[24px]">
            ...
            {activeTab === 'prs' && (
              ...
              >
                {exercisePRs.length === 0 ? (
                  <p className="text-[10px] font-black text-slate-500 text-center py-10 uppercase tracking-widest border border-white/10 border-dashed rounded-[32px]">Sem recordes no momento.</p>
                ) : (
                  exercisePRs.map((pr, idx) => (
                    <div key={idx} className="bg-surface border border-white/5 p-5 rounded-[24px] shadow-lg relative overflow-hidden">
                      {idx === 0 && (
                      ...
                      <div className="flex items-center gap-3 mb-4">
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center font-black text-lg shadow-inner ${idx === 0 ? 'bg-neon-blue/20 text-neon-blue' : 'bg-white/5 text-slate-400'}`}>
                          {idx === 0 ? '🏆' : idx + 1}
                        </div>
                        <span className={`font-black text-base max-w-[70%] truncate ${idx === 0 ? 'text-neon-blue' : 'text-white'}`}>{pr.exerciseName}</span>
                      </div>

                      <div className="grid grid-cols-2 gap-3">
                        <div className="bg-background/80 p-3 rounded-2xl flex flex-col items-center justify-center border border-white/5">
                          <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1 mt-1">Carga Máxima</span>
                          <span className={`text-xl font-black ${idx === 0 ? 'text-white' : 'text-white'}`}>{pr.maxWeight} kg</span>
                        </div>
                        <div className="bg-background/80 p-3 rounded-2xl flex flex-col items-center justify-center border border-white/5">
                          <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1 mt-1">Sessões / Volume</span>
                          <span className="text-xl font-black text-white">{pr.maxReps} r</span>
                        </div>
                      </div>
                    </div>
                  ))
