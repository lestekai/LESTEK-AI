...
export default function WorkoutsPage() {
  ...
  return (
    <div className="min-h-screen bg-background pb-28 overflow-x-hidden font-sans">
      ...
      <main className="px-5 sm:px-0 max-w-xl mx-auto space-y-8 mt-2">
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="flex items-center justify-between">
        <div>
           <h2 className="text-2xl font-black text-white tracking-tight font-display">Pronto para o topo?</h2>
      ...
      <section>
        <motion.div 
          ...
          className="relative rounded-[32px] overflow-hidden bg-surface border border-white/5 shadow-2xl"
        >
           ...
           <div className="relative p-6 z-10 flex flex-col min-h-[200px]">
              <div className="flex justify-between items-start mb-5">
                 <div>
                    <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-white/10 text-white rounded-full text-[10px] font-bold tracking-wider mb-2 backdrop-blur-md border border-white/5">
                    ...
                    <h3 className="text-2xl font-black text-white leading-none tracking-tight mt-1">
                 ...
                 {!todayPlan.isRest && (
                   <button onClick={(e) => { e.stopPropagation(); navigate('/workouts/progression'); }} className="w-10 h-10 flex items-center justify-center bg-white/5 hover:bg-white/15 text-white transition-colors rounded-full backdrop-blur border border-white/5 shadow-lg">
                      <List size={18} />
                   </button>
                 )}
              </div>
              ...
              {!todayPlan.isRest && (
                <div className="mt-auto pt-4 flex flex-col gap-4 border-t border-white/10">
                   <div className="flex items-center justify-between">
                     <span className="text-xs font-bold text-slate-400">Progresso da Fase</span>
                   ...
                   <button 
                     ...
                     className="w-full mt-2 py-4 bg-neon-blue rounded-2xl text-black font-black text-sm flex items-center justify-center gap-2 shadow-[0_8px_30px_rgba(0,210,255,0.3)] hover:brightness-110 active:scale-[0.98] transition-all"
                   >
                   ...
                   <button 
                     ...
                     className="w-full py-3 bg-transparent border-2 border-dashed border-white/10 text-slate-400 rounded-2xl font-black text-xs uppercase tracking-widest flex items-center justify-center gap-2 hover:border-white/30 hover:text-white transition-all"
                   >
                     <Edit3 size={16} />
              ...
              {todayPlan.isRest && (
                <div className="mt-auto pt-5">
                  <div className="bg-neon-blue/10 border border-neon-blue/20 px-5 py-4 rounded-2xl flex items-center gap-4">
                    ...
                    <div>
                      <p className="font-bold text-base text-white">Dia Off</p>
                      <p className="text-slate-400 text-xs font-medium mt-0.5">Aproveite para focar em alongamento ou cardio leve.</p>
                    </div>
                  </div>
                </div>
              )}
      ...
      <section>
        <div className="flex items-center justify-between mb-5">
           <h2 className="text-xl font-black text-white font-display tracking-tight flex items-center gap-2">
             ...
             {userTemplates && userTemplates.length > 0 && (
               <span className="text-xs font-bold text-slate-500 bg-surface border border-white/5 px-2 py-0.5 rounded-full font-mono">
        ...
        <div className="flex gap-4 overflow-x-auto pb-6 -mx-5 px-5 custom-scrollbar snap-x snap-mandatory">
          {/* CREATE CARD */}
          <button 
            onClick={() => navigate('/workouts/free')} 
            className="snap-start shrink-0 w-[270px] h-[195px] bg-gradient-to-br from-surface/50 to-surface border-2 border-dashed border-neon-purple/20 hover:border-neon-purple/50 rounded-[32px] flex flex-col items-center justify-center p-6 text-center hover:bg-neon-purple/5 active:scale-95 transition-all group relative overflow-hidden"
          >
             {/* Neon Glow on hover */}
             <div className="absolute inset-0 bg-neon-purple/2 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none" />
             <div className="w-14 h-14 rounded-full bg-neon-purple/10 group-hover:bg-neon-purple flex items-center justify-center mb-4 transition-all duration-300 shadow-inner group-hover:scale-110">
                <Plus size={24} className="text-neon-purple group-hover:text-black transition-colors duration-300" />
             </div>
             <p className="text-white font-black text-base tracking-tight mb-1">Criar Treino Extra</p>
             <p className="text-slate-500 text-[11px] font-medium max-w-[200px] leading-snug"> Monte livremente ou use inteligência artificial para gerar.</p>
          </button>
          ...
          {(userTemplates || []).map((tpl, i) => {
            ...
            return (
              <div 
                 key={`user-${i}`}
                 className="snap-start shrink-0 w-[270px] h-[195px] relative group"
              >
                ...
                <div className="absolute top-4 right-4 z-20 flex gap-2">
                  <button 
                    ...
                    className="w-8 h-8 rounded-full bg-black/50 hover:bg-neon-blue/20 text-white/40 hover:text-neon-blue backdrop-blur-md flex items-center justify-center border border-white/5 hover:border-neon-blue/30 transition-all active:scale-90"
                  >
                    <Edit3 size={14} />
                  </button>
                  <button 
                    ...
                    className="w-8 h-8 rounded-full bg-black/50 hover:bg-red-500/20 text-white/40 hover:text-red-400 backdrop-blur-md flex items-center justify-center border border-white/5 hover:border-red-500/30 transition-all active:scale-90"
                  >
                    <Trash2 size={14} />
                  </button>
                </div>
                ...
                <button 
                  ...
                  className="w-full h-full text-left bg-surface rounded-[32px] border border-white/5 relative overflow-hidden p-6 flex flex-col justify-between hover:border-white/20 hover:bg-surface-light transition-all shadow-xl active:scale-95 duration-200"
                >
                  ...
                  <div className="mt-2 flex-1 flex flex-col justify-center">
                    <h3 className="font-black text-white text-base leading-tight tracking-tight line-clamp-1 group-hover:text-neon-blue transition-colors">
                      {tpl.phaseName}
                    </h3>
                    ...
                    {isSingleDay && tpl.schedule[0]?.exercises && (
                      <div className="flex flex-wrap gap-1 mt-2">
                        {tpl.schedule[0].exercises.slice(0, 2).map((ex: any, idx: number) => (
                          <span key={idx} className="text-[10px] bg-white/5 text-slate-300 px-1.5 py-0.5 rounded-md border border-white/5 max-w-[100px] truncate">
                            {ex.name}
                          </span>
                        ))}
      ...
      <section className="pb-4 space-y-3">
         ...
         <Link to="/workouts/free" className="bg-surface rounded-3xl border border-white/5 p-5 flex items-center gap-4 hover:bg-surface-light transition-all group">
            <div className="w-14 h-14 rounded-full bg-neon-blue/10 flex items-center justify-center shrink-0">
               <Zap size={24} className="text-neon-blue group-hover:scale-110 transition-transform" />
            </div>
            <div>
               <h3 className="text-base font-black text-white">Treino Personalizado</h3>
               <p className="text-slate-400 text-xs font-medium mt-1">Crie e monte treinos personalizados com IA.</p>
            </div>
         </Link>

         {/* 2 Meu Progresso */}
         <Link to="/workouts/history" className="bg-surface rounded-3xl border border-white/5 p-5 flex items-center gap-4 hover:bg-surface-light transition-all group">
            <div className="w-14 h-14 rounded-full bg-neon-purple/10 flex items-center justify-center shrink-0">
               <TrendingUp size={24} className="text-neon-purple group-hover:scale-110 transition-transform" />
            </div>
            <div>
               <h3 className="text-base font-black text-white">Meu Progresso</h3>
               <p className="text-slate-400 text-xs font-medium mt-1">Histórico, PRs e estatísticas da sua evolução.</p>
            </div>
         </Link>

         {/* 3 Treinos Prontos */}
         <Link to="/workouts/templates" className="bg-surface rounded-3xl border border-white/5 p-5 flex items-center gap-4 hover:bg-surface-light transition-all group">
            <div className="w-14 h-14 rounded-full bg-neon-blue/10 flex items-center justify-center shrink-0">
               <Sparkles size={24} className="text-neon-blue group-hover:scale-110 transition-transform" />
            </div>
            <div>
               <h3 className="text-base font-black text-white">Treinos Prontos</h3>
               <p className="text-slate-400 text-xs font-medium mt-1">Explore templates criados por especialistas.</p>
            </div>
         </Link>

         {/* 4 Biblioteca de Movimentos */}
         <Link to="/workouts/library" className="bg-surface rounded-3xl border border-white/5 p-5 flex items-center gap-4 hover:bg-surface-light transition-all group">
            <div className="w-14 h-14 rounded-full bg-white/5 flex items-center justify-center shrink-0">
               <Dumbbell size={24} className="text-slate-300 group-hover:text-white transition-colors" />
            </div>
            <div>
               <h3 className="text-base font-black text-white">Biblioteca de Movimentos</h3>
               <p className="text-slate-400 text-xs font-medium mt-1">Consulte pesos e execuções corretas de +100 exercícios.</p>
            </div>
         </Link>
      </section>
      ...
      <AnimatePresence>
        {confirmAction && (
          ...
          >
            ...
            >
              <h3 className={`text-xl font-black mb-2 flex items-center gap-2 ${confirmAction.isDestructive ? 'text-red-400' : 'text-white'}`}>
              ...
              <div className="flex flex-col gap-3">
                <button
                  ...
                  className={`w-full py-4 rounded-2xl font-black text-sm uppercase tracking-widest flex items-center justify-center gap-2 transition-transform active:scale-95 ${
...
function TemplateEditor({ templateId, initialDayIndex, onClose }: { templateId: string, initialDayIndex: number, onClose: () => void }) {
  ...
  return (
    <>
      ...
      />
      {isMultiDay && (
        <div className="fixed top-4 left-1/2 -translate-x-1/2 z-[60] bg-surface border border-white/10 rounded-full px-2 py-1 flex gap-1 shadow-2xl backdrop-blur-md">
          {template.schedule.map((_, i) => (
            <button
              ...
              className={`w-8 h-8 rounded-full font-bold text-xs flex items-center justify-center transition-all ${
                i === dayIndex ? 'bg-neon-blue text-black' : 'bg-transparent text-slate-400 hover:text-white'
              }`}
            >
              {i + 1}
            </button>
